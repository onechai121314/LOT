...
if ($showAll) {
    ...
    $query = "SELECT number, date, award_type FROM six_digit_numbers ORDER BY date DESC";
    $stmt = $db->prepare($query); // เตรียมคำสั่ง SQL
} else {
    // สร้างคำสั่ง SQL เพื่อตามหาเลขหกหลักตามเงื่อนไขที่เลือก
    $query = "SELECT number, date, award_type FROM six_digit_numbers WHERE 1=1"; // เพิ่มเงื่อนไขที่ 1=1 ให้คำสั่ง SQL เป็นที่สมบูรณ์
...
$latestNumber = '';
$formattedNumber = sprintf("%06d", $latestNumber); // จะได้ "012345"
...
// ดึงข้อมูลเลข 6 หลักล่าสุด
$queryLatest = "SELECT number FROM six_digit_numbers ORDER BY id DESC LIMIT 1";
...
$queryYears = "SELECT DISTINCT strftime('%Y', date) AS year FROM six_digit_numbers ORDER BY year DESC";
...
$queryMonths = "SELECT DISTINCT strftime('%m', date) AS month FROM six_digit_numbers ORDER BY month ASC";
...
$queryDays = "SELECT DISTINCT strftime('%d', date) AS day FROM six_digit_numbers ORDER BY day ASC";
...
while ($row = $resultDuplicates->fetchArray(SQLITE3_ASSOC)) {
    $duplicates[] = [
        'number' => sprintf("%06d", $row['number']), // รูปแบบตัวเลข 6 หลัก
...
incrementViewCount($db, 'lot_six');

// ดึงจำนวนคนดูในวันนี้
$queryViews = "SELECT view_count FROM lot_six_views WHERE view_date = :view_date";
...
$ids_to_fetch = [3, 4]; // กำหนดให้ดึงข้อมูลจาก id 1 และ 3
...
function predictNumbers($frequencies, $latestDigits) {
    ...
    // จัดรูปแบบเป็นเลข 6 หลัก
    while (count($predictedNumbers) < 6) {
        $predictedNumbers[] = rand(0, 9); // เติมเลขสุ่มถ้าครบ 6 หลักแล้ว
    }
        ...
        <?php if ($numbers): ?>
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        <td><?php echo sprintf("%06d", $row['number']); ?></td> <!-- แสดงเลข 6 หลัก โดยมีเลข 0 ข้างหน้า -->
                        <td>
                            <?php
...
document.getElementById('refreshButton').addEventListener('click', function() {
        // เรียกใช้งานข้อมูลจาก lot_six.php ด้วย AJAX
        fetch('lot_six.php?show_all=true')
