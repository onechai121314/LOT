...
if ($showAll) {
    ...
    $query = "SELECT number, date, award_type FROM two_digit_numbers ORDER BY date DESC";
    $stmt = $db->prepare($query); // เตรียมคำสั่ง SQL
} else {
    // สร้างคำสั่ง SQL เพื่อตามหาเลขหกหลักตามเงื่อนไขที่เลือก
    $query = "SELECT number, date, award_type FROM two_digit_numbers WHERE 1=1"; // เพิ่มเงื่อนไขที่ 1=1 ให้คำสั่ง SQL เป็นที่สมบูรณ์
...
$latestNumber = '';
$formattedNumber = sprintf("%02d", $latestNumber); // จะได้ "012345"
...
// ดึงข้อมูลเลข 2 หลักล่าสุด
$queryLatest = "SELECT number FROM two_digit_numbers ORDER BY id DESC LIMIT 1";
...
$queryYears = "SELECT DISTINCT strftime('%Y', date) AS year FROM two_digit_numbers ORDER BY year DESC";
...
$queryMonths = "SELECT DISTINCT strftime('%m', date) AS month FROM two_digit_numbers ORDER BY month ASC";
...
$queryDays = "SELECT DISTINCT strftime('%d', date) AS day FROM two_digit_numbers ORDER BY day ASC";
...
while ($row = $resultDuplicates->fetchArray(SQLITE3_ASSOC)) {
    $duplicates[] = [
        'number' => sprintf("%02d", $row['number']), // รูปแบบตัวเลข 6 หลัก
...
incrementViewCount($db, 'lot_two');

// ดึงจำนวนคนดูในวันนี้
$queryViews = "SELECT view_count FROM lot_two_views WHERE view_date = :view_date";
...
$ids_to_fetch = [9, 10]; // กำหนดให้ดึงข้อมูลจาก id 1 และ 3
...
function predictNumbers($frequencies, $latestDigits) {
    ...
    // จัดรูปแบบเป็นเลข 2 หลัก
    while (count($predictedNumbers) < 2) {
        $predictedNumbers[] = rand(0, 9); // เติมเลขสุ่มถ้าครบ 2 หลักแล้ว
    }
...
<head>
    ...
    <title>สถิติเลขท้าย 2 ตัว</title>
...
<div class="logo">
 ...
 <div class="advertisement">
   ...
   <main>
        ...
        <form method="get" action="lot_two.php">
            <label for="date">ค้นหาตามวัน/เดือน/ปี:</label>
            <select id="day-date" name="day">
                <option value="">-- ดูทั้งหมด --</option>
                <?php foreach ($days as $d): ?>
                    <option value="<?php echo sprintf('%02d', $d); ?>" <?php echo (sprintf('%02d', $d) == $day) ? 'selected' : ''; ?>>
            ...
            <select id="month-date" name="month">
                ...
                <?php foreach ($months as $m): ?>
                    <option value="<?php echo sprintf('%02d', $m); ?>" <?php echo (sprintf('%02d', $m) == $month) ? 'selected' : ''; ?>>
        ...
        <?php if ($numbers): ?>
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        <td><?php echo sprintf("%02d", $row['number']); ?></td> <!-- แสดงเลข 6 หลัก โดยมีเลข 0 ข้างหน้า -->
                        <td>
                            <?php
...
document.getElementById('refreshButton').addEventListener('click', function() {
        // เรียกใช้งานข้อมูลจาก lot_two.php ด้วย AJAX
        fetch('lot_two.php?show_all=true')
