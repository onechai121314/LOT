...
$ids_to_fetch = [1, 2]; // กำหนดให้ดึงข้อมูลจาก id 1 และ 3
...
<body>
 ...
 <div class="advertisement">
    ...
    <main>
        <section>
        <h1>ผลรางวัลประจำวันที่: <?= htmlspecialchars($latest_date_formatted) ?> จำนวนการเข้าหน้าเว็บวันนี้ : <?php echo $viewsToday; ?> ครั้ง </h1>
        <!-- </h1>  <h1>จำนวนการเข้าหน้าเว็บวันนี้ : <?php echo $viewsToday; ?> ครั้ง</h1> -->

        <div class="award-section">
        <h2>รางวัลเลข 6 ตัว:</h2>
        <?php foreach ($six_digit_awards['awards'] as $award): ?>
            <a href="lot_six.php?award_number=<?= htmlspecialchars($award['number']) ?>" class="award-item">
                <?= htmlspecialchars($award['number']) ?>
            </a>
        <?php endforeach; ?>
        </div>
        ...
        <div class="award-section">
            ...
            <?php foreach ($first_three_digit_awards['awards'] as $award): ?>
                <a href="lot_first_three.php?award_number=<?= htmlspecialchars($award['number']) ?>" class="award-item">
                    <?= htmlspecialchars($award['number']) ?>
                </a>
            <?php endforeach; ?>
        </div>
        ...
        <div class="award-section">
            ...
            <?php foreach ($last_three_digit_awards['awards'] as $award): ?>
                <a href="lot_last_three.php?award_number=<?= htmlspecialchars($award['number']) ?>" class="award-item">
                    <?= htmlspecialchars($award['number']) ?>
                </a>
            <?php endforeach; ?>
        </div>
        ...
        <div class="award-section">
            ...
            <?php foreach ($two_digit_awards['awards'] as $award): ?>
                <a href="lot_two.php?award_number=<?= htmlspecialchars($award['number']) ?>" class="award-item">
                    <?= htmlspecialchars($award['number']) ?>
                </a>
            <?php endforeach; ?>
        </div>
        ...
        <!-- <div class="award-section">
            <h2>รางวัลเลขท้าย 2 ตัว:</h2>
            <?php foreach ($two_digit_awards['awards'] as $award): ?>
                <div class="award-item"> <?= htmlspecialchars($award['number']) ?> | รางวัล: <?= htmlspecialchars($award['award_type']) ?></div>
            <?php endforeach; ?>
        </div> -->
    ...
    <!-- Footer -->
    <!-- <footer>
        <p>&copy; 2024 บริษัทตัวอย่าง | สงวนลิขสิทธิ์</p>
    </footer> -->
