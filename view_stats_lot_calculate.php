...
// ดึงข้อมูลจำนวนคนครั้งในแต่ละวัน ของ lot_six.php
$query = "SELECT view_date, view_count FROM lot_calculate_views ORDER BY view_date DESC";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete'])) {
    ...
    $stmt = $db->prepare("DELETE FROM lot_calculate_views WHERE view_date = :view_date");
...
<head>
    ...
    <title>สถิติการเข้าชมหน้าเว็บ</title>
...
<div class="container">
    ...
    <div class="sidebar-left">
        <div class="menu">
            <a href="admin_page.php">หน้าแรก(สำหรับผู้ครั้งแล)</a>
            <a href="form.php">กรอกข้อมูล</a>
            <a href="view.php">ครั้งข้อมูล</a>
    ...
    <h1>สถิติการเข้าชมหน้าเว็บของ เลขออกซ้ำ</h1>
    
    <!-- แสดงผลในรูปแบบกราฟ -->
    <h2>ข้อมูลการเข้าชมหน้าเพจ (กราฟ)</h2>
    <canvas id="viewsChart" width="400" height="200"></canvas>

    <!-- แสดงผลในรูปแบบตาราง -->
    <h2>ข้อมูลการเข้าหน้าเพจ (ตาราง)</h2>
    <table border="1">
        <thead>
            <tr>
                <th>วันที่</th>
                <th>จำนวนครั้งในการเข้าหน้าเว็บ</th>
            </tr>
        </thead>
        <tbody>
    ...
    <script>
        ...
        const viewsChart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    label: 'จำนวนครั้ง',
                    data: <?php echo json_encode(array_column($LotTwoViews, 'view_count')); ?>, // จำนวนคนครั้ง
            ...
            options: {
                ...
                plugins: {
                    legend: { display: true },
                    title: { display: true, text: 'จำนวนชมต่อครั้งในแต่ละวัน (กราฟแท่ง)' }
                }
            }
        });
    </script>
