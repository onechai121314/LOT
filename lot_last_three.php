...
if ($showAll) {
    ...
    $query = "SELECT number, date, award_type FROM last_three_digit_numbers ORDER BY date DESC";
    $stmt = $db->prepare($query); // เตรียมคำสั่ง SQL
} else {
    // สร้างคำสั่ง SQL เพื่อตามหาเลขหกหลักตามเงื่อนไขที่เลือก
    $query = "SELECT number, date, award_type FROM last_three_digit_numbers WHERE 1=1"; // เพิ่มเงื่อนไขที่ 1=1 ให้คำสั่ง SQL เป็นที่สมบูรณ์
...
$queryLatest = "SELECT number FROM last_three_digit_numbers ORDER BY id DESC LIMIT 1";
...
$queryYears = "SELECT DISTINCT strftime('%Y', date) AS year FROM last_three_digit_numbers ORDER BY year DESC";
...
$queryMonths = "SELECT DISTINCT strftime('%m', date) AS month FROM last_three_digit_numbers ORDER BY month ASC";
...
$queryDays = "SELECT DISTINCT strftime('%d', date) AS day FROM last_three_digit_numbers ORDER BY day ASC";
...
// ตัวอย่างการเรียกใช้ฟังก์ชันเมื่อเข้าชมหน้า lot_last_three.php
incrementViewCount($db, 'lot_last_three');

// ดึงจำนวนคนดูในวันนี้
$queryViews = "SELECT view_count FROM lot_last_three_views WHERE view_date = :view_date";
...
$ids_to_fetch = [7, 8]; // กำหนดให้ดึงข้อมูลจาก id 1 และ 3
...
<head>
    ...
    <title>สถิติเลขท้าย 3 ตัว</title>
...
<div class="logo">
 ...
 <div class="advertisement">
   ...
   <main>
        ...
        <form method="get" action="lot_last_three.php">
...
document.getElementById('refreshButton').addEventListener('click', function() {
        // เรียกใช้งานข้อมูลจาก lot_last_three.php ด้วย AJAX
        fetch('lot_last_three.php?show_all=true')
